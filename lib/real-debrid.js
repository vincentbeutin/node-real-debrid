...
var connect = function(user, password, callback) {
    var connectOptions = {
        url: 'https://real-debrid.fr/ajax/login.php?user=' + user + '&pass=' + password + '&captcha_challenge=&captcha_answer=&time=' + (new Date().getTime()),
        jar: true,
...
var debrid = function(user, password, link, callback) {
    var debridOptions = {
        url: 'https://real-debrid.fr/ajax/unrestrict.php?link=' + link + '&password=&remote=1&time=' + (new Date().getTime()),
        jar: true,
        followAllRedirects: true,
        encoding: 'utf-8',
        headers: {
