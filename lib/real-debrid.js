...
var connect = function(user, password, callback) {
    var connectOptions = {
        url: 'https://real-debrid.com/ajax/login.php?user=' + user + '&pass=' + password + '&captcha_challenge=&captcha_answer=&time=' + (new Date().getTime()) + '&pin_challenge=&pin_answer=',
        jar: true,
...
var debrid = function(user, password, link, callback) {
    var debridOptions = {
        url: 'https://real-debrid.com/ajax/unrestrict.php?link=' + link + '&password=',
        jar: true,
        followAllRedirects: true,
        encoding: 'utf-8',
        headers: {
